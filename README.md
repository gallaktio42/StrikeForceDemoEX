...
2. Попросит восстановить пакеты NuGet (попробу сначала наверху окна найти Сборка -> Очистить решение -> Пересобрать решение -> Собрать решение) ИЛИ попробуй dotnet clean->dotnet restore (эти комангды вводишь в терминал, терминал открывается так: ![alt text](image.png) (правой кнопкой мыши по DemoExam(справа должно быть окно Обозревателя Решений)->И ищешь пункт ОТКРЫТЬ В ТЕРМИНАЛЕ))
...
если эксперты будут говорить про вторую ветку ГИТА, то
прежде чем писать команды, начиная от git add ., пишешь git checkout -b develop (создаешь и сразу же переключаешься на новую ветку)